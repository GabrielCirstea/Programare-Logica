...
ciur_fis(Numar,ListaNrPrime,ExtensieFisier):- ciur(Numar,ListaNrPrime), working_directory(W,''), atom_concat(W,'primele_pana_la_',A),
					atom_concat(A,Numar,X), atom_concat(X,'.',Y), atom_concat(Y,ExtensieFisier,Z), 
					tell(Z), write(ListaNrPrime), told.